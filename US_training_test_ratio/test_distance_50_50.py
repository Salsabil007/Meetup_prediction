...
#last er 8652 ta row ar shurur 8652 ta row alada kore disi
...
all_user = pd.read_csv("alluser_final_withtime_location.csv")
...
data = data.head(train_len) ###correction
data_2 = data_2.head(train_len) ###correction
data = data.append(data_2, ignore_index = True)
dupp = data
...
###correction
new_data1 = data_save.tail(test_len)
new_data2 = data2_save.tail(test_len)
...
data = dupp.head(train_len)
data = data.append(dupp2.head(test_len), ignore_index = False)
...
### for end data
data_2 = dupp.tail(train_len)
data_2 = data_2.append(dupp2.tail(test_len), ignore_index = False)
...
def process_test(df, model,nn,cc,data_no_scale,backup2,test_2,data_no_scale_2,a_user,backup):
    ...
    for i in x:
        ...
        if (out == np.argmax(yhat)):
            ...
            # if (is_in_topk(act[len-1][10],score,10) == 1):
            #     in_10 += 1
            #     fc_10 += 1

            # if (is_in_topk(act[len-1][10],score,20) == 1):
            #     in_20 += 1
            #     fc_20 += 1
            
            # if (is_in_topk(act[len-1][10],score,3) == 1):
            #     in_3 += 1
            #     fc_3 += 1
            
            # if (is_in_topk(act[len-1][10],score,1) == 1):
            #     in_1 += 1
            #     fc_1 += 1
    ...
    # print("total in top ten ",in_10)
    # print("percentage in top ten ",(in_10/total) * 100.00)
    # print("first section true ",fc_10," second section true ", in_10 - fc_10)

    # print("total in top twenty ",in_20)
    # print("percentage in top twenty ",(in_20/total) * 100.00)
    # print("first section true ",fc_20," second section true ", in_20 - fc_20)

    # print("total in top three ",in_3)
    # print("percentage in top three ",(in_3/total) * 100.00)
    # print("first section true ",fc_3," second section true ", in_3 - fc_3)

    # print("total in top one ",in_1)
    # print("percentage in top one ",(in_1/total) * 100.00)
    # print("first section true ",fc_1," second section true ", in_1 - fc_1)
...
def training_model(df,model,nn,data_no_scale,backup2,data_no_scale_2,train_2):
    print("training model e dhukseeee")
    x = df['userid']
    x = x.unique()
    X,y = [],[]
    m,d,p,yr,grp,dt,near_cls_grp,clsr_day,clstr_period,pp,clstr_frnd=[],[],[],[],[],[],[],[],[],[],[]
...
embed_1=Embedding(input_dim=32,output_dim=15,input_length=maxlen)(input_1)
...
process_test(test, model,nn,cluster_centers,data_no_scale,backup2,test_2,data_no_scale_2,a_user,backup)#first half er jonne
process_test3(test, model,nn,cluster_centers,data_no_scale,backup2,test_2,data_no_scale_2)#second half er jonne
print("US test distance finalized all user")
